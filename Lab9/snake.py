...
class Food:
    def __init__(self):
        self.position = (0, 0)
        self.color = RED
        self.weight = random.choice([1, 1, 1, 2, 2, 3])  # Different weights for food
        self.timer = 100  # Time until the food disappears
    ...
    def update_timer(self):
        ...
        if self.timer <= 0:
            self.randomize_position()
            self.timer = 100  # Reset the timer
...
def main():
    ...
    while True:
        ...
        if snake.get_head_position() == food.position:
            snake.score += food.weight * SCORE_PER_FOOD
            if snake.score % 30 == 0:  # Increase level every 3 foods eaten
                snake.level += 1
